...
4. **Governança:** run id / versão do modelo alinhada ao **MLflow** (ou registro equivalente); mesmas premissas do [Model Card](model-card.md) sobre `TELCO_SKLEARN_PIPELINE_PATH` vs modelo de pesquisa.
...
- **Artefato:** mesmo `.joblib` ou formato exportado consumido hoje pela variável `TELCO_SKLEARN_PIPELINE_PATH` (ou evolução ONNX/TorchScript para MLP, se decidido).
